...
stereo_pairs = ['lr', 'ud', 'ur']  # left-right, up-down, up-right
...
class Dataset3D60Fusion(Dataset):
  #360D Dataset#
  def __init__(self,
               filenamesFile,
    ...
    #########################################################################################################
    # Initialization
    super(Dataset3D60Fusion, self).__init__()
    # Assertion
    assert curStage in splits
    assert (rootDir is not None) and (rootDir != '')
    assert pair in stereo_pairs
  ...
  def __getitem__(self, index):  #return data in disparity estimation task
    ...
    if self.pair == 'lr':
      ...
      depth = leftDepthName
      depth_r = rightDepthName
      pred_depth_1 = left.replace(self.prefix_l, self.prefixPredDepth_l)[:-4] + '_disp_pred2depth.npz'
      pred_depth_2 = right.replace(self.prefix_r, self.prefixPredDepth_r)[:-4] + '_disp_pred2depth.npz'
      conf_map_1 = left.replace(self.prefix_l, self.prefixConfMap_l)[:-4] + '_conf_map.png'
      conf_map_2 = right.replace(self.prefix_r, self.prefixConfMap_r)[:-4] + '_conf_map.png'
      rotate_vector = np.array([0, 0, 0]).astype(np.float32)
    elif self.pair == 'ud':
      left = upName
      right = leftName
      depth = upDepthName
      depth_r = leftDepthName
      pred_depth_1 = left.replace(self.prefix_u, self.prefixPredDepth_u)[:-4] + '_disp_pred2depth.npz'
      pred_depth_2 = right.replace(self.prefix_l, self.prefixPredDepth_l)[:-4] + '_disp_pred2depth.npz'
      conf_map_1 = left.replace(self.prefix_u, self.prefixConfMap_u)[:-4] + '_conf_map.png'
      conf_map_2 = right.replace(self.prefix_l, self.prefixConfMap_l)[:-4] + '_conf_map.png'
      rotate_vector = np.array([0, 0, -np.pi / 2]).astype(np.float32)
    elif self.pair == 'ur':
      left = upName
      right = rightName
