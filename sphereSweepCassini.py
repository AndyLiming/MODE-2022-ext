  ...
  def genCassiniSweepGrids(self):
    ...
    for i in range(self.cam_nums):
      ...
      for d in range(numInvs_2):
        ...
        X2 = torch.matmul(R0, (X1 - t0)).view(3, h, w)  # DEBUG 1->other (+); other->1(-)
...
if __name__ == '__main__':
  ...
  #---------------------------------------------------------------#
  # NOTE: following testing trans view-1 to other views
  rootDir = '../../datasets/Deep360/ep1_500frames/testing'
  leftName = os.path.join(rootDir, 'rgb', '002505_12_rgb1.png')
  depthName = os.path.join(rootDir, 'depth', '002505_depth.npz')
  img1 = cv2.imread(leftName).astype(np.float32)
  depth1 = np.load(depthName)['arr_0'].astype(np.float32)
